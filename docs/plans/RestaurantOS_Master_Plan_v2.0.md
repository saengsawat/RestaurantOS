...
| **E25** | Roles & visibility | ✅ Done 2026-09-04 (D33/D35: four levels enforced server-side, every role sees its own app, the back-office hub landed with it) | Permission enum grows to owner/manager/kitchen/server; one visibility matrix served with the session, enforced server-side; each role's app shows only its screens | E17, E21 | Owner outranks manager; kitchen and server lateral; titles stay vocabulary (D28) | Route refusals tested per role | Opus (core) + Sonnet (UI) |
...
**Status as of 2026-09-04** (20 completed, 1 in progress, 5 in queue). Where the non-clean rows stand:
