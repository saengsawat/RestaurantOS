...
| **E23** | Reservations | ⬜ In queue (spec first, D27) | The call-in book: staff-entered promises the floor shows and the host can override; online booking integrate-never-build pending Matt | E6, E20 | A reservation warns, never hard-locks; phone feeds the guestbook lookup rung | Spec signed off; deck E answered | Sonnet (spec), build TBD |
| **E24** | Team & labor | 🔄 In progress (D28: directory done 2026-09-01; rung 3 hours export building per D29; scheduling waits on Matt; payroll never computed) | People directory (title separate from permission, PII manager-gated), then planned-vs-actual scheduling, then hours/tips CSV export for payroll providers | E15, E21 | Contact details behind a manager PIN; wage math never done here | Directory live; deck F answered before scheduling | Opus (directory) + Sonnet (spec) |

**Status as of 2026-09-02** (17 completed, 2 in progress, 5 in queue). Where the non-clean rows stand:
