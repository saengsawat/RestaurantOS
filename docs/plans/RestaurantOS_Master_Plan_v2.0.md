...
| **E24** | Team & labor | 🔄 In progress (D28: directory builds now, scheduling waits on Matt, payroll never computed) | People directory (title separate from permission, PII manager-gated), then planned-vs-actual scheduling, then hours/tips CSV export for payroll providers | E15, E21 | Contact details behind a manager PIN; wage math never done here | Directory live; deck F answered before scheduling | Opus (directory) + Sonnet (spec) |
