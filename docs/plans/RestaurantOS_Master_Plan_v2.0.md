...
| **E19** | Insights v1 (server report + heatmap) | 🔄 In progress | Per-server scorecard + hour/day sales heatmap as read-only ledger projections (D19; Phase 6 slice pulled forward) | E15 | Reports computed on read, never stored; per-server sums conserve against the day summary | Insights totals equal the close-day summary to the cent | Opus (core) + Sonnet/Codex (UI) |
...
**Status as of 2026-08-23** (12 completed, 3 in progress, 3 in queue). Where the non-clean rows stand:
...
- **E19 insights**: T1 (server attribution + the two read APIs) and T2 (the /insights page: tiles, server scorecard, heatmap) both merged 2026-08-23. The remainder is T3, a small fix from T2's review: the Tips tile's declared figure must come from a top-level total over all shifts, because scorecard rows exist only for employees who closed a check. T3 closes the epic.
