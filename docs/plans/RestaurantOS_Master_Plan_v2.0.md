...
| **E5** | Menu/config domain | ✅ Completed 2026-09-01 (D29: full manager-editable menu graph via draft/publish; relational draft storage deferred until a real import demands it) | Versioned menu graph, publishing snapshots | E3, E4 | Editing draft never mutates published snapshot **[T]** | Publish → order → edit menu → yesterday's check unchanged | Opus |
...
| **E22** | Migration / onboarding | ✅ v0 done 2026-09-02 (D30: menu CSV import live end to end, spec + core + UI; vendor converters wait for a real export file, staff/guest CSVs parked) | Import a switching restaurant: menu CSV into draft-then-publish, staff CSV with fresh PINs, floor redrawn | E5, E21 | Imports land as drafts, idempotent, source-audited; no sales history or gift-card liabilities in v1 | Spec signed off against a real customer export | Sonnet (spec), build TBD |
| **E23** | Reservations | ✅ Done 2026-09-03 (D31: spec, engine, and screens; a call becomes a booking, a floor badge, and a seated check; online booking stays integrate-never-build) | The call-in book: staff-entered promises the floor shows and the host can override; online booking integrate-never-build pending Matt | E6, E20 | A reservation warns, never hard-locks; phone feeds the guestbook lookup rung | Spec signed off; deck E answered | Sonnet (spec), build TBD |
| **E24** | Team & labor | ✅ Done 2026-09-03 (D28/D31: all three rungs built, directory + scheduling + hours export; payroll never computed, and the suite asserts the absence) | People directory (title separate from permission, PII manager-gated), then planned-vs-actual scheduling, then hours/tips CSV export for payroll providers | E15, E21 | Contact details behind a manager PIN; wage math never done here | Directory live; deck F answered before scheduling | Opus (directory) + Sonnet (spec) |
| **E25** | Roles & visibility | 🔄 Core merged 2026-09-03 (D33: four levels enforced server-side, matrix served with the session; UI next, batched with the D35 hub) | Permission enum grows to owner/manager/kitchen/server; one visibility matrix served with the session, enforced server-side; each role's app shows only its screens | E17, E21 | Owner outranks manager; kitchen and server lateral; titles stay vocabulary (D28) | Route refusals tested per role | Opus (core) + Sonnet (UI) |
| **E26** | Inventory & ingredients | ⬜ Spec done 2026-09-03, awaiting the FOUNDER's domain review (D34; his sign-off gates the build) | Ingredient stock with per-serving recipe links, derived depletion from kitchen fires, par levels, the chef's order list, the 86-proposal bridge; bar by station | E5, E8 | Depletion derived never stored; counts reset the baseline; no dollars in v1 | Founder signs off the spec before any build | Sonnet (spec), build TBD |

**Status as of 2026-09-03** (19 completed, 2 in progress or queued-with-build-fired, 5 in queue). Where the non-clean rows stand:
