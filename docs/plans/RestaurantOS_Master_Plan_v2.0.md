...
| **E24** | Team & labor | 🔄 In progress (D28/D31: directory done 2026-09-01, hours export done 2026-09-01, rung 2 scheduling build fired 2026-09-02; payroll never computed) | People directory (title separate from permission, PII manager-gated), then planned-vs-actual scheduling, then hours/tips CSV export for payroll providers | E15, E21 | Contact details behind a manager PIN; wage math never done here | Directory live; deck F answered before scheduling | Opus (directory) + Sonnet (spec) |
...
| **E26** | Inventory & ingredients | ⬜ In queue (D34, spec first; the founder is the domain reviewer) | Ingredient stock with per-serving recipe links, derived depletion from kitchen fires, par levels, the chef's order list, the 86-proposal bridge; bar by station | E5, E8 | Depletion derived never stored; counts reset the baseline; no dollars in v1 | Founder signs off the spec before any build | Sonnet (spec), build TBD |

**Status as of 2026-09-03** (18 completed, 3 in progress or queued-with-build-fired, 5 in queue). Where the non-clean rows stand:
