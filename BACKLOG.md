...
| **E8-T2** KDS/check drift guardrail | Founder-reported: a closed check's unbumped ticket sat red-late on the KDS with no hint the table settled, and nothing ever forces rail cleanup. Fix is visibility + sweep, never auto-clear: `check closed` chip on the KDS (late escalation suppressed for settled tables), and open kitchen tickets become a DAY-CLOSE BLOCKER named by table. Policy question (one-tap serve-remaining on close?) goes to the Matt deck. Contract: `docs/tickets/E8-T2-kds-check-drift.md` | **Ready** · Opus, cross-model review |
...
Suite total: 107 tests green (70 domain + 37 server incl. PostgreSQL integration).
