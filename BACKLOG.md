...
| **E11-T4** cross-partition guard | Server-side fix for the T3 worker's finding: a labeled portion payment must also cap at the CHECK's remaining due, or a second terminal can overpay a check whose money arrived under another partition's labels. UI already refuses; the server must too. Contract: `docs/tickets/E11-T4-cross-partition-guard.md` | **Ready** · Opus, cross-model review |
...
Suite total: 104 tests green (70 domain + 34 server incl. PostgreSQL integration).
