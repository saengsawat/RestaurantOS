...
The orchestrator (Fable) plans, writes ticket contracts under `docs/tickets/`, assigns models, and reviews; worker sessions build, ONE at a time (all sessions share this folder). Merged so far: E11-T1 through T4, E8-T2, and all of E19 (T1 attribution + read APIs, T2 the /insights page, T3 the declared-tips fix found in T2's review). Epic E19 closed 2026-08-23: the reporting slice is live end to end. E20-T1 (the guestbook spec) merged the same day. The founder then approved building the guestbook v0 now (D23) and renaming the reporting tab to Reports (D24, "Insights" is reserved for the AI layer). E20-T2 (guestbook core: the two tables, the commands, and the derived profile) reviewed and merged 2026-08-23. Next fire: ONE Sonnet/Codex session batching E19-T4 (rename to Reports) then E20-T3 (guestbook screens) per D22; that pair finishes everything currently approved. Then the queue waits on Matt or on an E5-full ticket. One-liner to fire a ticket: "You are a worker session. Execute the ticket at docs/tickets/<name>.md exactly; it is your entire scope."
